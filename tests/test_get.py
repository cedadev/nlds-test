...
class TestGet:
    ...
    def test_get_1(
        ...
        response = nlds_client.get_filelist(
            [filepath], target=target, job_label="test_get_1"
        )
    ...
    def test_get_1a(
        ...
        filepath = get_readable_path(1).as_posix()
        # delete to make room
        response = nlds_client.get_filelist([filepath], job_label="test_get_1a")
        state = wait_completed(response=response)
        assert state == "COMPLETE"
    ...
    def test_get_2(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], target=target, job_label="test_get_2"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_3(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], label="test_holding_1", target=target, job_label="test_get_3"
        )
        state = wait_completed(response=response)
        assert state == "COMPLETE"
    ...
    def test_get_4(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], label="test_holding_1", target=target, job_label="test_get_4"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_5(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], label="test_holding_1", target=target, job_label="test_get_5"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_6(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], holding_id=1, target=target, job_label="test_get_6"
        )
        state = wait_completed(response=response)
        assert state == "COMPLETE"
    ...
    def test_get_7(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], holding_id=1, target=target, job_label="test_get_7"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_8(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], holding_id=3, target=target, job_label="test_get_8"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_9(
        ...
        target = get_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], holding_id=3, target=target, job_label="test_get_9"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_10(
        ...
        filepath = get_readable_path(1).as_posix()
        target = get_target_dir().as_posix()
        job_label = "test_get_10"
        response = nlds_client.get_filelist(
            [filepath], target=target, job_label=job_label
        )
        state = wait_completed(response=response)
        assert state == "COMPLETE"
    ...
    def test_get_11(
        ...
        target = get_target_dir().as_posix()
        job_label = "test_get_11"
        response_1 = nlds_client.get_filelist(
            [filepath_1], target=target, job_label=job_label
        )
        response_2 = nlds_client.get_filelist(
            [filepath_2], target=target, job_label=job_label
        )
    ...
    def test_get_12(
        ...
        response = nlds_client.get_filelist(
            [filepath], target=target, tag=tag, job_label="test_get_12"
        )
        state = wait_completed(response=response)
        assert state == "COMPLETE"
    ...
    def test_get_13(
        ...
        response = nlds_client.get_filelist(
            [filepath], target=target, tag=tag, job_label="test_get_13"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_14(
        ...
        tag = {"filelist": "11 to 15"}
        response = nlds_client.get_filelist(
            [filepath], target=target, tag=tag, job_label="test_get_14"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_18(
        ...
        tag = {"filelist": "6 to 10"}
        response = nlds_client.get_filelist(
            [filepath], holding_id=2, target=target, tag=tag, job_label="test_get_18"
        )
        state = wait_completed(response=response)
        assert state == "COMPLETE"
    ...
    def test_get_19(
        ...
        tag = {"filelist": "6 to 10"}
        response = nlds_client.get_filelist(
            [filepath], holding_id=1, target=target, tag=tag, job_label="test_get_19"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_20(
        ...
        tag = {"filelist": "6 to 10"}
        response = nlds_client.get_filelist(
            [filepath], holding_id=1, target=target, tag=tag, job_label="test_get_20"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_21(
        ...
        response = nlds_client.get_filelist(
            [filepath], label="test_holding_1", target=target, job_label="test_get_21"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
    ...
    def test_get_21a(
        ...
        target = get_unwriteable_target_dir().as_posix()
        response = nlds_client.get_filelist(
            [filepath], target=target, job_label="test_get_21a"
        )
        state = wait_completed(response=response)
        assert state == "FAILED"
